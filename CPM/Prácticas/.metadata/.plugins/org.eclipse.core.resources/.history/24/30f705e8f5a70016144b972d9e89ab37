...
//		}
	...
	private boolean isJugadaCorrecta(int i) {
		return ((corredorActivo.getPosicion() + getNumeroDado() >= i) && (i > corredorActivo.getPosicion()));
	}
	...
	public boolean resolverJugada(int i) {
		...
		if (isJugadaCorrecta (i)){
			corredorActivo.setPosicion(corredorActivo.getPosicion() + (i - corredorActivo.getPosicion()));
